...
import { File, Trash2, Upload, Undo2 } from "lucide-react";
...
import { useRetakeDocumentInMutation } from "@/hooks/data/retake.data";
...
export default function RetakeDialog({
  ...
  const retakeInMutation = useRetakeDocumentInMutation();
  ...
  const handleRetakeDocument = async () => {
    if (type === "in") {
      await retakeDocumentIn();
    } else {
      // Document out
      if (isFromRetakeModule) {
        await retakeDocumentOutInRetakeModule();
      } else {
        await retakeDocumentOutInDocumentModule();
      }
    }
  };

  const retakeDocumentIn = async () => {
    setInSubmit(true);
    try {
      const response = await retakeInMutation.mutateAsync({
        docId: documentId,
        comment: {
  ...
  const retakeDocumentOutInRetakeModule = async () => {
    setInSubmit(true);
    try {
  ...
  const retakeDocumentOutInDocumentModule = async () => {
    setInSubmit(true);
    try {
      const item = {
