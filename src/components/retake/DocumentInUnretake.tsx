...
interface DocumentInUnretakeProps {
  isOpen: boolean;
...
export default function DocumentInUnretakeModal({
  isOpen,
  onOpenChange,
  onClose,
  documentId,
  refetch,
}: DocumentInUnretakeProps) {
  const { toast } = useToast();
  const unretakeMutation = useUnretakeDocumentInMutation();
  ...
  const handleUnretakeDocument = async () => {
    if (!comment.comment && comment.attachments.length > 0) {
    ...
    setInSubmit(true);
    try {
      const response = await unretakeMutation.mutateAsync({
        docId: documentId,
  ...
  const isSubmitDisabled =
    (!comment.comment && comment.attachments.length > 0) || inSubmit;
        ...
        <DialogDescription>
          <div className="flex flex-col gap-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="comment">Lý do khôi phục</Label>
              <Textarea
                id="comment"
              ...
              />
              {!comment.comment && comment.attachments.length > 0 && (
