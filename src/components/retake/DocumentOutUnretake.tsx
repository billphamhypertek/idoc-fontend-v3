...
interface DocumentOutUnretakeProps {
  isOpen: boolean;
...
export default function DocumentOutUnretakeModal({
  isOpen,
  onOpenChange,
  onClose,
  documentId,
  ...
  const handleUnretakeDocument = async () => {
    setInSubmit(true);
    try {
      const response = await RetakeService.doUnretakeDocumentOut(
        documentId,
        comment.comment,
        comment.attachments
      );
