...
import { File, Trash2, Upload, RotateCcw } from "lucide-react";
...
import { useUnretakeDocumentInMutation } from "@/hooks/data/retake.data";
import { ToastUtils } from "@/utils/toast.utils";

interface UnretakeDialogProps {
  isOpen: boolean;
...
export default function UnretakeDialog({
  isOpen,
  onOpenChange,
  onClose,
  documentId,
  ...
  const unretakeInMutation = useUnretakeDocumentInMutation();
  ...
  const handleUnretakeDocument = async () => {
    // Validation for document in: must have comment if there are attachments
    if (type === "in" && !comment.comment && comment.attachments.length > 0) {
      ToastUtils.error("Phải nhập lý do khôi phục khi có tệp đính kèm.");
      return;
    }
    ...
    try {
      ...
      if (type === "in") {
        // Handle document in - only comment, no files in initial call
        response = await unretakeInMutation.mutateAsync({
          docId: documentId,
          comment: comment.comment,
        });
      } else {
        // Handle document out - comment and files together
        // Service expects retakeComment as object with comment property, and files array
        // But service code uses retakeComment.attachments, so we pass comment object
        response = await RetakeService.doUnretakeDocumentOut(
          documentId,
          { comment: comment.comment, attachments: comment.attachments },
          comment.attachments
        );
      }
      ...
      ToastUtils.success("Khôi phục văn bản thành công");
      ...
      ToastUtils.error("Có lỗi xảy ra khi khôi phục văn bản");
    } finally {
      setInSubmit(false);
    }
  };
  ...
  const isSubmitDisabled =
    (type === "in" && !comment.comment && comment.attachments.length > 0) ||
    inSubmit;
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Khôi phục</DialogTitle>
        </DialogHeader>
        <DialogDescription>
              ...
              {type === "in" &&
                ...
                comment.attachments.length > 0 && (
                  <p className="text-sm text-red-500">
                    Phải nhập lý do khôi phục khi có tệp đính kèm.
                  </p>
                )}
            </div>
        ...
        <DialogFooter>
          <Button
            onClick={handleUnretakeDocument}
