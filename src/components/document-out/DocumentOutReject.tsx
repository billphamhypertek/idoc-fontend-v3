...
import { useReturnDocument } from "@/hooks/data/document.data";
...
interface DocumentOutRejectProps {
  docId: string;
  onClose: () => void;
...
export default function DocumentReject({
  ...
  useEffect(() => {
    if (propListReceiveAndSend) {
      ...
      const load = async () => {
        try {
          const data = await DocumentService.getListSend(Number(docId));
          setListReceiveAndSend(data || []);
        } catch (e) {
          setListReceiveAndSend([]);
        }
      };
      if (docId && showRejectModal) load();
    }
  }, [docId, showRejectModal, propListReceiveAndSend]);
  ...
  const handleSubmit = async () => {
    ...
    // Nếu có danh sách người nhận từ props (document-in), sử dụng callback
    const hasUserList =
      propListReceiveAndSend && propListReceiveAndSend.length > 0;
    if (hasUserList && propOnSubmit) {
      if (!selectedUserId) {
    ...
    // Nếu không có danh sách người nhận từ props (document-out), cần chọn người nhận
    if (!hasUserList && !pid) {
      ToastUtils.error("Bạn cần chọn người nhận trả");
      return;
    }
