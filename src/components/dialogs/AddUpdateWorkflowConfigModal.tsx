...
interface AddUpdateWorkflowConfigModalProps {
  isOpen: boolean;
  onOpenChange: (open: boolean) => void;
  onSave: (workflowConfig: WorkflowConfigRequest) => void;
  loading?: boolean;
  isEdit?: boolean;
  workflowConfigData?: WorkflowConfigRequest | null;
  errorSubmit?: { [k: string]: string };
}

export default function AddUpdateWorkflowConfigModal({
  isOpen,
  onOpenChange,
  onSave,
  loading = false,
  ...
  workflowConfigData,
  errorSubmit,
}: AddUpdateWorkflowConfigModalProps) {
  ...
  const getFormConfigTypeListQuery = useGetFormConfigTypeListQuery();
  const [formData, setFormData] = useState<WorkflowConfigRequest>({
    name: "",
    orgId: 2,
    orgName: "Ban Cơ yếu Chính phủ",
    categoryId: getFormConfigTypeListQuery?.data?.[0]?.id || 0,
    active: true,
    content: "",
    nodes: [],
    org: null,
  ...
  useEffect(() => {
    if (isOpen) {
      if (isEdit) {
        setFormData(workflowConfigData as WorkflowConfigRequest);
        setSelectedItems([
          {
            type: "org",
            orgId: workflowConfigData?.orgId?.toString() || "",
            orgName: workflowConfigData?.org?.name || "",
            userId: null,
            userName: "",
            fullName: "",
            positionId: null,
  ...
  }, [isOpen, isEdit, workflowConfigData]);
  ...
  const validate = () => {
    ...
    if (!categoryId) {
      errorObj.categoryId = "Loại đối tượng không được để trống";
    }
    return errorObj;
  };
  ...
  return (
    <div>
        ...
        <DialogContent className="sm:max-w-[800px] max-h-[160vh] overflow-x-hidden">
          ...
          <div className="space-y-4 py-4">
            <div className="grid grid-cols-1 md:grid-cols-1 gap-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="typeWorkflow" className="font-bold">
                  Loại đối tượng<span className="text-red-500">*</span>
                </Label>
                <SelectCustom
                  options={
                    getFormConfigTypeListQuery?.data?.map((item) => ({
                      id: item.id.toString(),
                      name: item.name,
                    })) || []
                  }
                  ...
                  className={`w-full h-9${internalErrors.typeWorkflow ? " border-red-500 focus:border-red-500 ring-red-500" : ""}`}
                />
                {internalErrors.typeWorkflow && (
                  <div className="text-xs text-red-500 mt-1">
                    {internalErrors.typeWorkflow}
                  </div>
                )}
      ...
      <OrganizationTreeDialog
        ...
        users={[] as UserInformationWorkflowConfig[]}
      />
    </div>
  );
}
