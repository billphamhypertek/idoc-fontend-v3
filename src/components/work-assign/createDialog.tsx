...
import ChildWorkAssignDialog from "@/components/work-assign/CreateChildDialog";
...
const WorkAssignDialog: React.FC<WorkAssignDialogProps> = ({
  ...
  const [isOpenUpperChildWorkDialog, setIsOpenUpperChildWorkDialog] =
    useState<boolean>(false);
  const [isOpenLowerChildWorkDialog, setIsOpenLowerChildWorkDialog] =
    useState<boolean>(false);
  const [isOpenRelatedChildWorkDialog, setIsOpenRelatedChildWorkDialog] =
    useState<boolean>(false);
  ...
  const initialFollowers = useMemo(() => {
    ...
    const hasCurrentUser = initialFollowers.some(
      (f: any) => f.userId === UserInfo.id
    );
    ...
    const fallbackSelf = {
      user: {
        ...
        positionName: UserInfo.positionModel?.name,
        orgName: UserInfo.orgModel?.name,
      },
      type: 0,
      isExcute: false,
      isCombination: true,
    ...
    const result = hasCurrentUser ? initialFollowers : [fallbackSelf];

    setSelectedFollower(
      result.map((item: any) => item?.user?.fullName).join(", ") ?? ""
    );
    setAssignForm((prev) => {
      if (!prev) return prev;
      return {
  ...
  const { mutateAsync: doAttachTaskAssignAsync } = useAttachTaskAssign(
    isV2 ?? false
  );
  const { mutateAsync: doAddTransferAsync } = useDoAddTransfer(isV2 ?? false);
  ...
  const handleViewAttachment = useCallback(async (file: any) => {
    const sourceFile = file?.file ?? file;
    try {
      await viewFile(sourceFile, Constant.ATTACHMENT_DOWNLOAD_TYPE.TASK);
    } catch (error) {
      handleError(error);
    }
  }, []);
  ...
  // Reset all data when dialog closes
  useEffect(() => {
    if (!open) {
      ...
      setAssigneeName("");
      setAttachedFiles([]);
      setCurrentSelectedFileType(undefined);
      setValidFileAttr({
        validFiles: true,
        isValidFileSize: true,
        isValidExtension: true,
      ...
      setSelectedFollower(UserInfo.fullName || "");
  ...
  useEffect(() => {
    setAssignForm((prev) => {
      ...
      return {
        ...prev,
        taskRelateds:
          relatedWorkData && relatedWorkData.length > 0
            ? relatedWorkData?.map((item) => ({
                ...item,
                taskRelatedId: item.id,
              }))
            : [],
      };
    });
    ...
    if (open && parentTaskFromDetail) {
      setAssignForm((prev) => {
        ...
        return {
          ...
          taskName: parentTaskFromDetail?.title || "",
          description: parentTaskFromDetail?.description || "",
        };
      });
      setUpperLevelWorkData([parentTaskFromDetail]);
  ...
  useEffect(() => {
    if (open && documentDetail) {
      const taskDoc: any = {
        id: documentDetail.id || documentDetail.docId,
        numberOrSign: documentDetail.numberOrSign,
        docTypeName:
          documentType === Constant.HSTL_DOCUMENT_TYPE.VAN_BAN_DEN
            ? "Văn bản đến"
            : "Văn bản đi",
        preview: documentDetail.preview,
      };

      setAssignForm((prev) => ({
        ...prev,
        taskName: documentDetail.preview,
        description: documentDetail.preview,
        taskDocument: [taskDoc],
      }));

      setAttachedDocumentData([taskDoc]);
    }
  }, [open, documentDetail]);

  const validateRequiredFields = () => {
    if (!assignForm?.taskName) {
      ToastUtils.error("Vui lòng nhập tên công việc");
      return false;
    }
    if (!assignForm?.description) {
      ToastUtils.error("Vui lòng nhập mô tả");
      return false;
    }
    /* if (assignForm.taskFieldId === 0 || assignForm.taskFieldId == null) {
      ToastUtils.error("Vui lòng chọn Lĩnh vực công việc");
      return false;
    } */
    if (assignForm.priorityId === 0 || assignForm.priorityId == null) {
      ToastUtils.error("Vui lòng chọn Nhóm công việc");
      return false;
    }
    if (assignForm.complexityId === 0 || assignForm.complexityId == null) {
      ToastUtils.error("Vui lòng chọn Mức độ phức tạp");
      return false;
    }
    return true;
  };
  ...
  const getAssigneeNames = (assignees: TaskExecute[] | null): string => {
    if (!assignees || assignees.length === 0) return "";

    const names: string[] = [];
    assignees.forEach((assignee) => {
      if (assignee.user?.fullName) {
        names.push(assignee.user.fullName);
      }
    });
  ...
  const doAssignee = (assignees: TaskExecute[]) => {
    ...
    // Save to store for next time (only if node is selected)
    if (selectedNode?.id) {
      setLastSelectedNodeId(selectedNode.id);
      setLastAssignee(assignees, "");
    }
  };

  const loadNodes = async (
    type: boolean,
    shouldOpenPopover: boolean = false
  ) => {
  ...
  const openTransferModal = (node: Node | null = null) => {
    const nodeToUse = node || selectedNode;
    if (!nodeToUse) {
      // If no node provided, just show popover to select node
      setIsNodesPopoverOpen(true);
      return;
    }

    // If selecting a different node, clear store and previous assignee
    if (selectedNode && selectedNode.id !== nodeToUse.id) {
      setAssignee([]);
      setAssigneeName("");
      setLastAssignee([], "");
      setLastSelectedNodeId(null);
  ...
  return (
    <>
      ...
      >
        <CustomDialogContent
          className="sm:max-w-5xl max-h-[95vh] p-0 bg-gray-50 flex flex-col overflow-hidden"
          aria-modal="true"
          aria-label="Giao việc"
          onInteractOutside={(e) => e.preventDefault()}
        >
          <DialogHeader className="px-4 py-3 bg-white border-b border-gray-200 flex-shrink-0">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Button
                  variant="outline"
                  onClick={() => {
                    // Clear store when clicking Cancel button
                    setLastAssignee([], "");
                    setLastSelectedNodeId(null);
                    onClose();
                  }}
          ...
          <div className="p-4 space-y-4 relative overflow-y-auto flex-1 min-h-0">
            {/* Thông tin chi tiết */}
            <Card className="p-2">
                      ...
                      >
                        <PopoverTrigger asChild>
                          <div
                            ...
                            onClick={() => {
                              if (!isNodesPopoverOpen) {
                                loadNodes(false, true);
                              }
                            }}
                            title={`${selectedNode?.name} - ${getAssigneeNames(assignee)}`}
                          >
                            <Input
                              id="assignee"
                              ...
                              value={
                                selectedNode
                                  ? assignee && assignee.length > 0
                                    ? `${selectedNode.name} - ${getAssigneeNames(assignee)}`
                                    : selectedNode.name
                                  : "---Chọn---"
                              }
                              type="text"
                              className="h-9 bg-white border-gray-300 rounded-lg pl-10 pr-8 text-sm transition-colors w-full cursor-pointer [&:read-only]:!bg-white"
                              placeholder="Chọn người xử lý việc"
                              readOnly
                            />
                            <User
                              className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-500 pointer-events-none"
                ...
                <CardContent className="space-y-4 p-2 pt-3">
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <Button
                        className="flex items-center gap-1 h-9 px-3 text-xs bg-blue-600 hover:bg-blue-700 rounded-md font-medium transition-colors shadow-sm hover:shadow-md"
                        onClick={() => setIsOpenUpperChildWorkDialog(true)}
                      >
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-3">
                        ...
                        <div className="flex flex-col gap-0.5">
                          ...
                          <div className="mt-1">
                            <button
                              ...
                              className="text-xs text-blue-600 underline hover:text-blue-800 focus:outline-none"
                              onClick={() =>
                                setIsOpenLowerLevelWorkDialog(true)
                              }
                            >
                      ...
                      <Button
                        className="flex items-center gap-1 h-9 px-3 text-xs bg-blue-600 hover:bg-blue-700 rounded-md font-medium transition-colors shadow-sm hover:shadow-md"
                        onClick={() => setIsOpenLowerChildWorkDialog(true)}
                      >
                        <Plus className="w-3 h-3" aria-hidden="true" />
                        Thêm công việc
                      </Button>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <Button
                        className="flex items-center gap-1 h-9 px-3 text-xs bg-blue-600 hover:bg-blue-700 rounded-md font-medium transition-colors shadow-sm hover:shadow-md"
                        onClick={() => setIsOpenRelatedChildWorkDialog(true)}
                      >
                        <Plus className="w-3 h-3" aria-hidden="true" />
                        Thêm công việc
                      </Button>
      ...
      <FollowerDialog
        ...
        initialFollowers={assignForm.userFollows || initialFollowers}
        onConfirm={(followers) => {
      ...
      {!isV2 && (
        <ChildWorkAssignDialog
          open={isOpenUpperChildWorkDialog}
          onAssign={(data) => setUpperLevelWorkData([data])}
          onClose={() => setIsOpenUpperChildWorkDialog(false)}
          title="Tạo công việc mức trên"
          type="upper"
          isV2={isV2}
        />
      )}
      {!isV2 && (
        <ChildWorkAssignDialog
          open={isOpenLowerChildWorkDialog}
          onAssign={(data) =>
            setLowerLevelWorkData([...lowerLevelWorkData, data])
          }
          onClose={() => setIsOpenLowerChildWorkDialog(false)}
          title="Tạo công việc mức dưới"
          type="lower"
          isV2={isV2}
        />
      )}
      {!isV2 && (
        <ChildWorkAssignDialog
          open={isOpenRelatedChildWorkDialog}
          onAssign={(data) => setRelatedWorkData([...relatedWorkData, data])}
          onClose={() => setIsOpenRelatedChildWorkDialog(false)}
