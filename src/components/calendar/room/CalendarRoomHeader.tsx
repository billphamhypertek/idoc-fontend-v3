...
import {
  useExportRoomFileAction,
  useImportRoomFileAction,
} from "@/hooks/data/calendar.actions";
import { saveAs } from "file-saver";
import { ToastUtils } from "@/utils/toast.utils";

interface CalendarRoomHeaderProps {
  onAddRoom?: () => void;
}

export default function CalendarRoomHeader({
  onAddRoom,
}: CalendarRoomHeaderProps) {
  const fileInputRef = useRef<HTMLInputElement>(null);
  const exportRoomFileMutation = useExportRoomFileAction();
  const importRoomFileMutation = useImportRoomFileAction();
  ...
  const handleFileSelected = async (
    ...
    try {
      ...
      await importRoomFileMutation.mutateAsync(formData);
  ...
  const handleExportFile = async (type: "excel" | "csv") => {
    try {
      const response = await exportRoomFileMutation.mutateAsync(type);
      ...
      const fileName = `Danh_sach_phong_hop_${timestamp}.${
        type === "excel" ? "xlsx" : "csv"
      }`;
    ...
    <div className="bg-gray-100 px-4 py-4 rounded-lg mb-5 flex justify-between items-center">
      ...
      <div className="flex gap-2">
        <Button
          ...
          disabled={importRoomFileMutation.isPending}
        >
        ...
        <DropdownMenu>
          ...
          >
            <DropdownMenuItem
              ...
              disabled={exportRoomFileMutation.isPending}
            >
              Export ra Excel
            </DropdownMenuItem>
            <DropdownMenuItem
