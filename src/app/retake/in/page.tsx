...
import RetakeInFilter from "@/components/retake/RetakeInFilter";
...
import { getDefaultRetakeInSearchField } from "@/utils/formValue.utils";
...
export default function RetakeIn() {
  ...
  const deleteDraftOutMutation = useDeleteDraftOut();
  const pageSizeRef = useRef<number>(
    getDefaultRetakeInSearchField().pageSize || Constant.ITEMS_PER_PAGE
  );

  const { data: initialDocumentData, refetch: refetchInitial } =
    useGetDocumentIn(searchField.page, !isSearchEnabled);
  ...
  useEffect(() => {
    if (initialDocumentData?.data) {
      setSearchField((prev) => ({
        ...prev,
        pageSize: searchField.pageSize || Constant.ITEMS_PER_PAGE,
      }));
      setSearchList(initialDocumentData.data.content);
    }
  }, [initialDocumentData, searchField.pageSize]);

  useEffect(() => {
    // Luôn đồng bộ ref kích thước trang với state mới nhất
  ...
  // Bỏ khởi tạo riêng; sẽ đồng bộ hoàn toàn theo URL trong effect dưới

  // Cập nhật SearchFilter mỗi khi URL params thay đổi
  useEffect(() => {
    ...
    // Map size/page từ URL vào searchField.pageSize/page
    ...
    // Thực thi tìm kiếm theo page hiện có trên URL
    doSearch(parsedPage);
  }, [searchParams]);
  ...
  const doAdvanceSearch = async (
    ...
    // Dùng ref để tránh lấy nhầm pageSize của nhịp trước
    ...
    pushWithParams({
      ...
      orgName: searchField.orgIssuedName || "",
  ...
  const handleResetSearch = () => {
    setSearchField(getDefaultRetakeInSearchField());
    setSearchList([]);
  ...
  const handleSearch = (searchData: any) => {
    ...
    const params = {
      text: searchData.quickSearchText || "",
      sortBy: searchField.sortBy || "",
      direction: searchField.direction || "DESC",
  ...
  const handleAdvancedSearch = (searchData: any) => {
    ...
    const params: any = {
      ...
      page: 1,
      docTypeId: searchData.docTypeId || searchField.docTypeId || "",
      orgName: searchData.orgIssuedName || searchField.orgIssuedName || "",
      userEnter: searchData.userEnter || searchField.userEnter || "",
      sortBy: searchField.sortBy || "",
      direction: searchField.direction || "DESC",
      size: searchField.pageSize || 10,
      retaked: currentTab === TabNames.DATHUHOI,
  ...
  const handlePageSizeChange = (pageSize: number) => {
    // Cập nhật ref trước để các hàm search lấy đúng size mới
  ...
  const handleViewDocument = (document: any) => {
    ...
    router.push(`/retake/in/detail/${document.docOutId}`);
  };
  ...
  const handleSort = (field: string) => {
    ...
    switch (field) {
      case "numberOrSign":
      case "Số/Ký hiệu":
        sortField = SearchTitle.NUMBERSIGN;
        break;
      case "preview":
      case "Trích yếu":
      ...
      case "docTypeName":
      case "Loại văn bản":
        sortField = SearchTitle.DOCTYPE;
        break;
      case "dateIssued":
      case "Ngày ban hành":
        sortField = SearchTitle.DATEISSUED;
        break;
      default:
  ...
  const handleSelectionChange = (selectedKeys: React.Key[]) => {
    if (selectedKeys.length > 0) {
      const targetItem = searchList.find(
        (doc) => doc.docOutId === selectedKeys[0]
      );
      ...
      setSearchList((prevList: any[]) => {
        return prevList.map((item: any) => {
          if (item.docOutId === selectedKeys[0]) {
  ...
  const handleRetakeDocument = (document: any) => {
    setCurrentDocumentId(document.docOutId);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await deleteDraftOutMutation.mutateAsync(documentToDelete);
  ...
  const columns = [
      ...
      accessor: (item: any, index: number) => (
        <div className="flex items-center justify-center gap-2">
          ...
          <Input
            ...
            onChange={(e) => {
              e.stopPropagation();
              handleSelectionChange([item.docOutId]);
            }}
    ...
    },
    {
      ...
      accessor: (item: any) => {
        const numberInBook = item.numberInBook || "";
        const numberOrSign = item.numberOrSign || "";
        const displayText =
          numberOrSign?.indexOf(numberInBook) >= 0
            ? numberOrSign
          ...
          return (
            <div className="text-center d-flex justify-content-center gap-2">
              ...
              <Button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  handleDeleteDocument(item.docOutId);
                }}
                className="bg-transparent shadow-none border-none hover:bg-transparent"
              >
  ...
  return (
    <div className="space-y-4 p-4">
      <BreadcrumbNavigation
        ...
        currentPage="Thu hồi văn bản đi"
        showHome={false}
      />
      <RetakeInFilter
        key={filterKey}
        ...
        docTypeCategory={doLoadDocTypeCategory}
      ...
      <ConfirmDeleteDialog
        ...
        confirmText="Đồng ý"
        cancelText="Đóng"
        isLoading={deleteDraftOutMutation.isPending}
      />
    </div>
  );
}
