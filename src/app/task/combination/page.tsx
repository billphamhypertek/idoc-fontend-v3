...
import { useTaskCombinationListV2 } from "@/hooks/data/taskv2.data";
...
export default function TaskWork() {
  ...
  const STORAGE_KEY = "task_combination_search_state";
  ...
  const renderTodoButton = (task: Task) => {
    const config: {
    ...
    }[] = [
      {
        ...
        title: "Nhận việc",
        color: "text-black",
        onClick: () => confirmApprove(task, 1),
        icon: <FileCheck className="w-4 h-5" />,
      },
      {
        key: "canTodo",
      ...
      },
      {
        ...
        onClick: () => doLoadUserFollowingTask(task),
      ...
      },
      {
        ...
        color: "text-cyan-500",
        onClick: () => openFamilyTreeModal(task),
          ...
          return (
            ...
            >
              <PopoverTrigger asChild>
                <div
                  className="flex items-center justify-center p-[2px] hover:bg-gray-200"
  ...
  const draftHandleColumns: Column<Task>[] = [
    ...
    },
    {
      header: (
        <div className="flex justify-center items-center py-1 cursor-pointer hover:text-blue-700">
          <span className="font-bold">Người giao việc</span>
        </div>
      ),
    ...
    },
    {
      header: (
        <div className="flex justify-start items-center py-1 cursor-pointer hover:text-blue-700">
          <span className="font-bold">Nhóm công việc</span>
        </div>
      ),
  ...
  const {
    ...
    error,
    refetch,
  } = useTaskCombinationList(activeTab, params, shouldSearch && !isV2);
  ...
  const {
    ...
    error: errorV2,
    refetch: refetchV2,
  } = useTaskCombinationListV2(
    activeTab,
    params,
    shouldSearch && (isV2 ?? false)
  );
  ...
  useEffect(() => {
    // Chỉ cập nhật displayedData khi shouldSearch là true (đã click tìm kiếm, thay đổi input khác, hoặc chuyển trang)
  ...
  const checkNavigateDetail = (task: Task) => {
    if (task?.id !== undefined && task?.id !== null) {
      if (isV2) {
        router.push(`/task-v2/combination/detail/${task.id}`);
      } else {
        router.push(`/task/combination/detail/${task.id}`);
      }
    }
  };
  ...
  const acceptTask = async () => {
    ...
    try {
      ...
      //router.push(`/task/combination/detail/${taskSelectedAction.id}`);
    } catch (error) {
      handleError(error);
    }
  };
  ...
  const loadNodes = async (task: Task, type: boolean) => {
    ...
    try {
      const taskDetail = await TaskService.findById(task.id);
      let data = [];
  ...
  const handlePageChange = (page: number) => {
    setCurrentPage(page);
    setShouldSearch(true); // Trigger search khi chuyển trang
  };

  const search = () => {
    setCurrentPage(1);
    setShouldSearch(true);
    refetchMerged(); // Đảm bảo call API khi click tìm kiếm
  };
  ...
  return (
    <div className="px-4 space-y-4 lg:space-y-8">
      <BreadcrumbNavigation
        currentPage="Việc phối hợp xử lý"
        showHome={false}
        items={[
          {
            href: "",
        ...
        <div className="flex items-center justify-between">
          <div className="flex-1">
            ...
            <p className="text-sm text-gray-600">
              Danh sách công việc với vai trò phối hợp xử lý
            </p>
          </div>
        </div>
      </div>
