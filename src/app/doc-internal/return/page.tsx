...
const defaultSearchField = {
  currentTab: Constant.DOC_INTERNAL_TAB_INDEX.RETURN,
  numberOrSign: "",
  preview: "",
...
export default function DocInternalReturnPage() {
  ...
  // Query data for RETURN tab
  const { data: returnData, isLoading: isReturnLoading } =
    useGetListDocInternal({
      tab: String(Constant.DOC_INTERNAL_TAB_INDEX.RETURN),
  ...
  const gotoDetailPage = (doc: any) => {
    ...
    router.push(`/doc-internal/return/detail/${doc.id}`);
  };
    ...
    <div className="pl-4 pr-4 space-y-4">
      ...
      >
        <h2 className="text-lg font-semibold">
          Danh sách văn bản yêu cầu sửa lại
        </h2>
      </div>
      ...
      <Table
        columns={columns}
        dataSource={returnData?.content || []}
        loading={isReturnLoading}
        totalItems={returnData?.totalElements || 0}
        itemsPerPage={searchField.pageSize}
        currentPage={searchField.page}
        onPageChange={pageChanged}
        onItemsPerPageChange={changePageSize}
        pageSizeOptions={Constant.PAGE_SIZE_OPTION?.map((item) => item.value)}
