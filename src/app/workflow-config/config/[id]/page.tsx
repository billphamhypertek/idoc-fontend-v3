...
import {
  useGetWorkflowConfigDetailById,
...
import { WorkflowConfigRequest } from "@/definitions/types/workflow-config.type";
...
export default function ProcessConfigPage() {
  ...
  const { mutate: updateProcessDetail } = useUpdateWorkflowConfigDetail();
  ...
  } = useGetWorkflowConfigDetailById(processId);
  const queryClient = useQueryClient();
  const isInitializedRef = useRef(false);

  // Ref to always have the latest processData in listeners
  ...
  // Refs to manage update state
  const lastImportedXMLRef = useRef<string>("");
  const isManualUpdatingRef = useRef(false);
  ...
  useEffect(() => {
    if (processId && !isFetching && !isProcessDataLoading) {
      if (!processData) {
            ...
            const handleImportDone = (event: any) => {
              ...
              if (!error && modelerRef.current) {
                ...
                (eventBus as any).on(
                  "elements.changed",
                  async (_event: any) => {
                    if (isManualUpdatingRef.current) return;
                    if (saveTimeoutRef.current)
                      clearTimeout(saveTimeoutRef.current);

                    saveTimeoutRef.current = setTimeout(async () => {
                      if (!modelerRef.current || isManualUpdatingRef.current)
                        return;
                      try {
                        const xml = await getXML(modelerRef.current);
                        if (xml === lastImportedXMLRef.current) return;
  ...
  }, [processId, isFetching, isProcessDataLoading, processData, refreshKey]);
  ...
  const handleUpdateNode = async (updatedData: ProcessRequest) => {
    try {
      ...
      isManualUpdatingRef.current = true;
      updateProcessDetail(
        {
          payload: {
            ...(updatedData as WorkflowConfigRequest),
            content: newXml,
            id: processId,
          },
        },
        {
          onSuccess: async () => {
            lastImportedXMLRef.current = newXml;
            await refetch();
            isManualUpdatingRef.current = false;
            handleCloseDialog();
            ToastUtils.success("Cập nhật luồng thành công");
          },
          onError: (error) => {
    ...
    <div className="h-screen flex flex-col">
      ...
      <div className="flex-1 relative overflow-hidden p-2">
        ...
        <ConfigNodeDialog
          ...
          process={processData as WorkflowConfigRequest}
