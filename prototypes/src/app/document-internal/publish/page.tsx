...
import { documentInternalPublish } from "@/data/document-internal-data";
import { cn } from "@/lib/utils";
import { CheckCircledIcon } from "@radix-ui/react-icons";
import { Send } from "lucide-react";

const tabs = [
    { id: "pending", label: "Chờ phát hành", count: 1 },
    { id: "done", label: "Đã phát hành", count: 0 },
];

export default function DocumentInternalPublishPage() {
    const router = useRouter();
    const [activeTab, setActiveTab] = useState("pending");
    const [selectedIds, setSelectedIds] = useState<string[]>([]);
    const [data, setData] = useState<DocumentItem[]>(documentInternalPublish);

    const filteredData = data.filter((item) => {
        if (activeTab === "pending") return item.status !== "issued";
    ...
    return (
        <PageLayout activeModule="doc-internal" activeSubMenu="publish">
            <div className="space-y-4">
                {/* Header */}
                <h1 className="text-xl font-semibold text-[hsl(var(--v3-card-foreground))]">Phát hành văn bản</h1>
