...
import { documentInternalApprove } from "@/data/document-internal-data";
...
const tabs = [
    ...
    { id: "done", label: "Đã duyệt", count: 0 },
];

export default function DocumentInternalApprovePage() {
    const router = useRouter();
    const [activeTab, setActiveTab] = useState("pending");
    const [selectedIds, setSelectedIds] = useState<string[]>([]);
    const [data, setData] = useState<DocumentItem[]>(documentInternalApprove);
    ...
    return (
        <PageLayout activeModule="doc-internal" activeSubMenu="approve">
            <div className="space-y-4">
                {/* Header */}
                <h1 className="text-xl font-semibold text-[hsl(var(--v3-card-foreground))]">Phê duyệt văn bản</h1>
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {hasSelection && (
                            <>
                                <button className="inline-flex items-center gap-2 h-9 px-4 rounded-lg border border-red-500 bg-white text-red-500 text-sm font-medium hover:bg-red-50 transition-colors">
                                    <Cross2Icon className="w-4 h-4" />
                                    Từ chối
                                </button>
                                <button className="inline-flex items-center gap-2 h-9 px-4 rounded-lg bg-green-500 text-white text-sm font-medium hover:bg-green-600 transition-colors shadow-sm">
